...
namespace mc {
    ...
    public:
        ...
        // This creates an array of objects of type T - those objects are default-constructed in the array
        // normally, only the memory should be allocated here (so no object construction should take place):
        explicit vector(std::size_t capacity):
                m_data{ static_cast<pointer>( ::operator new(capacity * sizeof(T)) ) }, // thanks to @zaldawid
                m_cap{ capacity },
                m_sz{ 0 } {}
        ...
        // Copy constructor
        vector(const vector& other): m_data{ static_cast<pointer>( ::operator new(other.capacity() * sizeof(T)) ) },
...
//                delete[] m_data; // release resource in *this
        ...
        void push_back(reference& entry) {
            ...
            ::new (m_data[m_sz++]) T{ std::move(entry) };
        }
